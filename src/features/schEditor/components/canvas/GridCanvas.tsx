...
export default function GridCanvas({
  ...
  const { minorSpacing, majorSpacing, renderMinorPx, minorLineColor, majorLineColor } = useGrid();
  ...
  useEffect(() => {
    ...
    // Draw minor grid lines (use colors from GridContext so theme changes apply)
    ctx.lineWidth = Math.max(1 / scale, 0.0001);
    ctx.strokeStyle = minorLineColor ?? "rgba(0,0,0,0.06)";
    ...
    ctx.strokeStyle = majorLineColor ?? "rgba(0,0,0,0.12)";
