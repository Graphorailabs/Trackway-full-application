  ...
  const { setSelectedSymbol, setSymbolData, setPendingSymbol } = useSymbol();
   ...
   const loadSymbolNames = (content: string) => {
    ...
    const handleContent = (id: string, content: string) => {
          ...
          // Do NOT set pendingSymbol here — wait until user clicks OK to arm placement
          console.log('selected symbol loaded (not armed for placement yet)', selectedSymbol )
    ...
    return (
      ...
      <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
              <div className="bg-[#0E0E0E] w-[90%] h-[85vh] rounded-xl shadow-2xl flex flex-col border border-gray-700 min-h-0">
     ...
     <div className="flex flex-1 bg-black divide-x divide-gray-700 min-h-0">
      ...
      <aside className="w-[20%] bg-[#181818] text-gray-200 flex flex-col min-h-0">
        <h2 className="text-xs font-semibold px-4 py-2 border-b border-gray-700">
          Symbols
        </h2>

        <ul className="text-xs flex-1 min-h-0 overflow-y-auto
          [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {symbolNames.map((name, i) => (
            <li
              key={i}
              className="cursor-pointer px-4 py-2 hover:bg-[#222]"
              onClick={() => {
          ...
          <div className="mt-4 flex justify-end gap-2">
            ...
            <button
              onClick={() => {
                  ...
                  // Set the pending symbol for placement (do NOT auto-place)
                  setPendingSymbol(getContentById.unit ?? getContentById);
                  setSymbolData(getContentById);
                  setSelectedSymbolId(getContentById.id);

                  // switch to symbol tool so user can position the preview
