...
export const SymbolPreviewCanvas = ({ symbolData, selected = false, ownerId, previewPins, onPinClick }: any) => {
  ...
  const RenderPin = ({ pin, idx, pins }: any) => {
    ...
    return (
      <>
        ...
        <Text x={pinNumberX} y={pinNumberY} text={String(pin.number?.[''] ?? '')} fontSize={PIN_NUMBER_FONT} fill="red" align="center" />
        ...
        {(() => {
          ...
          const estWidth = Math.max(8 * SCALE, txt.length * (PIN_NAME_FONT * SCALE * 0.6));
          let startX = pinNameX;
          ...
          const barY = pinNameY - (PIN_NAME_FONT * SCALE * 0.9);
  ...
  const selectionVisual = (() => {
    ...
    return (
      <>
        <Rect x={x} y={y} width={w} height={h} stroke={stroke} strokeWidth={0.8} dash={[6, 4]} listening={false} />
        {/* corner handles */}
